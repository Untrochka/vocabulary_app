...
**Not built yet:**
- **Evening recall mode**: explain a word learned that day in your own words, graded by Groq, feeding back into the active track as a review. New screen and flow, not started.
- **Nothing in the UI shows priority, debt, grading status, or open practice batches yet** — it all drives the queue now, but there's no visible indicator (a debt badge, a priority list, batch progress) anywhere on screen. Planned as part of the final screens pass alongside recall mode.
